...
public interface ExpLogRepo extends JpaRepository<ExpLog,Long> {
    ...
    @Query("SELECT new com.ssafy.newspeak.explog.repo.ExpLogDto(e.id, e.change) " +
            "FROM ExpLog e JOIN e.user u WHERE e.user.id = :userId")
    List<ExpLogDto> findAllExpLogByUserId(Long userId);

    @Query("SELECT new com.ssafy.newspeak.explog.repo.DateChangeDto(e.createdDate, SUM(e.change)) " +
            "FROM ExpLog e " +
    ...
    @Query("SELECT new com.ssafy.newspeak.explog.repo.DailyExpDto(e.createdDate, SUM(e.change)) " +
            "FROM ExpLog e " +
